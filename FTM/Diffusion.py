...
# in a frequency transformed scheme with an analytic solution to the precursor equations
    ...
    ############################
        ...
        while time < options.t_end: 
            ...
            for r in range(options.repeat_inner+1):
                ...
                # Only store values of rho, frequencies during the last repetition
                ...
                while time < InnerEnd:
                    ...
                    if time == 1.4467 and r == options.repeat_inner:
                        results.power_2d(options, Flux, XS, name='peak', amp=Amplitude.B[0])
                        results.temps_2d(options, Amplitude.T, name='peak')

                # Calculate frequencies - last inner step
                wp = np.log(Amplitude.stored_power[-1]/Amplitude.stored_power[-2])/options.subStep
                if store_w:
                ...
                sol = SolveTD(options, Flux, C, kcrit)
        ...
        results.frequencies(t_outer, stored_OmegaP)
