...
# in a hybrid alpha-eigenvalue scheme with optional repeated time steps

import time as runtime

from cross_sections import *
from solver_alpha import *
    ...
    ###########################
        ...
        while time < options.t_end: 
            ...
            for r in range(options.repeat_inner+1):
                ...
                Matrix.update_spatial_freq(options, XS, kcrit, 0.0, Amplitude.wd1, Amplitude.wd2)
                sol.hill_alg(options, Matrix.D, Matrix.O, Matrix.F, XS)
                Flux = copy.copy(sol.flux)
                LastEndFlux = copy.copy(EndFlux)
                EndFlux = copy.copy(sol.flux)
                ERR = max(abs(LastEndFlux[:]-EndFlux[:]))
                stored_kbalance[CountOuter] = sol.k
                stored_a[CountOuter] = sol.a
                # Note: sol.a replaces wp as a frequency
                Matrix.update_spatial_freq(options, XS, kcrit, sol.a, Amplitude.wd1, Amplitude.wd2)
                sol.gauss_seidel(options, Matrix.D, Matrix.Otrans, Matrix.Ftrans, XS)
                AdjFlux = copy.copy(sol.flux)
