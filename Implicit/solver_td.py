...
###############################################################
	...
	def gauss_seidel(self, opts, D, O, XS, start_flux, time):
		...
		# Update S vector
		SumC = np.dot(XS.DECAYi*np.exp(-XS.DECAYi[:]*opts.dt),self.C)
		fission_term = self.flux[:N]*XS.fis[:N]*sum(
			(XS.Bi[:]*XS.DECAYi[:])/XS.DECAYi[:]/self.kcrit * 
			(-np.exp(-XS.DECAYi[:]*opts.dt)+(1-np.exp(-XS.DECAYi[:]*opts.dt))/(XS.DECAYi[:]*opts.dt)))
